...
def fault_case(args, folder, fts, operation, fault, once=False):
    with Volume(folder) as volume:
        ...
        if args.mode == "io-errors":
            env.update(ZVEC_FAULT_OPERATION=fault, ZVEC_FAULT_AFTER="1")
            if once:
        ...
        try:
            run_worker(args, database, "verify", fts, folder)
        except Exception as error:
        ...
        for limit in cuts:
            ...
            try:
                ...
                run_worker(
                    ...
                    fts,
                    detail,
                    minimum=128 if limit >= durable else 64,
                )
            except Exception:
                error = traceback.format_exc()
    ...
    for fts in (False, True):
        ...
        for operation in ("insert", "flush", "optimize"):
            if args.mode == "power-loss":
                ...
                fault = "sync" if operation == "flush" else "write"
                for once in (True, False):
                    suite.case(
                        f"{kind}-{operation}-{'once' if once else 'persistent'}",
                        partial(
                            fault_case,
                            args,
                            fts=fts,
