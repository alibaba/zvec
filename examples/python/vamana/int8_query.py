"""
Zvec Python API — Vamana INT8 Query Example
=============================================

Opens a Vamana INT8 collection (built by int8_build.py), runs vector
searches, and evaluates recall against ground truth.

The reformer (CosineInt8Reformer) is automatically loaded from the stored
index meta during collection.open(), which converts query vectors using the
same quantization parameters used during build.

Configuration:
    Collection : /root/data/cohere/1m/db/cohere_vamana_cosine_int8
    TopK       : 100
...
def main() -> None:
    print("=" * 60)
    print("  Zvec Python API — Vamana INT8 Query Example")
    print("=" * 60)
    ...
    result_file = "/tmp/vamana_int8_first_results.txt"
