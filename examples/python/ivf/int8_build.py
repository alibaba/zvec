"""
Zvec Python API — IVF INT8 Build Example
=========================================

Builds a zvec Collection with IVF index and INT8 quantization (no rotation).

Input : /root/data/cohere/1m/cohere_train_vector_1m.norm.zvec.vecs
Output: /root/data/cohere/1m/db/cohere_ivf_cosine_int8
...
VECS_FILE = "/root/data/cohere/1m/cohere_train_vector_1m.norm.random.zvec.vecs"
COLLECTION_PATH = "/root/data/cohere/1m/db/cohere_ivf_cosine_int8"
...
def main() -> None:
    print("=" * 60)
    print("  Zvec Python API — IVF INT8 Build Example")
    print("=" * 60)
    ...
    print(f"  quantize_type = QuantizeType.INT8 (no rotation)")
    print(f"  metric_type   = MetricType.L2")
    print(f"  n_list        = {N_LIST}")
    print(f"  n_iters       = {N_ITERS}")
    print(f"  use_soar      = {USE_SOAR}")
    ...
    # ---- Step 5: Optimize (build IVF index with INT8) ----
    print(f"\n[Step 5] Optimizing collection (building IVF index with "
          f"INT8) ...")
