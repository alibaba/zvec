"""
Zvec Python API — IVF INT8 + Random Rotation Query Example
============================================================

Opens an IVF INT8 + rotation collection (built by int8_rotate_build.py),
runs vector searches, and evaluates recall against ground truth.

The reformer is automatically loaded from the stored index meta during
collection.open(), which rotates query vectors using the same FhtKacRotator
that was used during build.

Configuration:
    Collection : /root/data/cohere/1m/db/cohere_ivf_cosine_int8_rotate
    TopK       : 100
...
def main() -> None:
    print("=" * 60)
    print("  Zvec Python API — IVF INT8 + Rotate Query Example")
    print("=" * 60)
