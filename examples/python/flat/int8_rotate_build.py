"""
Zvec Python API — Flat INT8 + Random Rotation Build Example
============================================================

Builds a zvec Collection with Flat (brute-force) index, INT8 quantization,
and random rotation (CosineInt8Converter + FhtKacRotator) enabled.
...
Output: /root/data/cohere/1m/db/cohere_flat_cosine_int8_rotate
...
def main() -> None:
    print("=" * 60)
    print("  Zvec Python API — Flat INT8 + Rotate Build Example")
    print("=" * 60)
    ...
    # ---- Step 3: Create collection with Flat + INT8 + enable_rotate ----
    print(f"\n[Step 3] Creating collection at {COLLECTION_PATH} ...")
    print(f"  index_type    = Flat (brute-force)")
    ...
    schema = zvec.CollectionSchema(
        name="cohere_flat_cosine_int8_rotate",
        fields=[
            FieldSchema(
                "id",
                DataType.INT64,
    ...
    # ---- Step 5: Optimize (build Flat index with INT8 + rotation) ----
    print(f"\n[Step 5] Optimizing collection (building Flat index with "
          f"INT8 + rotation) ...")
