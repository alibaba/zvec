"""
Zvec Python API — Flat INT8 + Random Rotation Query Example
=============================================================

Opens a Flat + INT8 + rotation collection (built by int8_rotate_build.py),
runs brute-force vector searches, and evaluates recall against ground truth.
...
Configuration:
    Collection : /root/data/cohere/1m/db/cohere_flat_cosine_int8_rotate
    TopK       : 100
...
COLLECTION_PATH = "/root/data/cohere/1m/db/cohere_flat_cosine_int8_rotate"
...
def main() -> None:
    print("=" * 60)
    print("  Zvec Python API — Flat INT8 + Rotate Query Example")
    print("=" * 60)
    ...
    for rnd in range(total_rounds):
        ...
        for idx, (key, vec) in enumerate(queries):
            ...
            if not is_warmup and results:
                first_results.append(results[0].id)
            elif not is_warmup:
    ...
    # ---- Step 6: Save first results ----
    result_file = "/tmp/flat_int8_rotate_first_results.txt"
    with open(result_file, "w") as f:
        for i, rid in enumerate(first_results):
            qid = queries[i][0] if queries[i][0] is not None else str(i)
            f.write(f"{qid}\t{rid}\n")
    print(f"\n[Step 6] Saved {len(first_results)} results to {result_file}")

    # ---- Step 6b: Compare with Flat INT8 (no-rotate) ----
    int8_result_file = "/tmp/flat_int8_first_results.txt"
    if os.path.exists(int8_result_file):
        print(f"\n[Step 6b] Comparing with Flat INT8 (no-rotate) ...")
        int8_results: dict[str, Optional[str]] = {}
        with open(int8_result_file, "r") as f:
            for line in f:
                parts = line.strip().split("\t", 1)
                if len(parts) == 2:
                    int8_results[parts[0]] = parts[1]

        same_count = diff_count = 0
        diffs: list[tuple[str, Optional[str], Optional[str]]] = []
        for i, rid in enumerate(first_results):
            qid = queries[i][0] if queries[i][0] is not None else str(i)
            int8_rid = int8_results.get(qid)
            if rid == int8_rid:
                same_count += 1
            else:
                diff_count += 1
                if len(diffs) < 10:
                    diffs.append((qid, int8_rid, rid))

        total_compared = same_count + diff_count
        print(f"  Compared {total_compared} queries:")
        print(f"    Same first result : {same_count} ({same_count / total_compared * 100:.1f}%)")
        print(f"    Different         : {diff_count} ({diff_count / total_compared * 100:.1f}%)")
        if diffs:
            print(f"  First few differences:")
            for qid, int8_rid, rot_rid in diffs:
                print(f"    query={qid}  int8={int8_rid}  rotate={rot_rid}")
    else:
        print(f"\n[Step 6b] Flat INT8 result file not found ({int8_result_file}),")
        print(f"          Run flat/int8_query.py first to enable comparison.")
