...
preprocessing (RotationPreprocessor with FHT-based Kac rotation).
...
This triggers the C++ pipeline to:
  ...
  3. Store the rotation matrix in the index storage for search-side query rotation
...
def main() -> None:
    ...
    # ---- Step 3: Create collection with INT8 + rotation preprocessing ----
    print(f"\n[Step 3] Creating collection at {COLLECTION_PATH} ...")
    print(f"  quantize_type = QuantizeType.INT8")
    print(f"  enable_rotate  = True")
    print(f"  metric_type  = MetricType.COSINE")
    print(f"  → RotationPreprocessor + INT8 quantization")
