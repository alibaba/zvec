...
This triggers the C++ CosineInt8Converter to:
  ...
  2. Store the rotator state in the index meta for search-side query rotation

Equivalent C++ config (construct2.yaml):
    ConverterName: CosineInt8Converter
    ConverterParams:
        integer_streaming.converter: !!bool true

Input : /root/data/cohere/1m/cohere_train_vector_1m.norm.zvec.vecs
Output: /root/data/cohere/1m/db/cohere_cosine_int8
...
def main() -> None:
    print("=" * 60)
    print("  Zvec Python API — INT8 + Build Example")
    print("=" * 60)
