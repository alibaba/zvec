/*
 * Zvec C API — INT8 + Random Rotation Build Example
 * ===================================================
 *
 * Builds a zvec Collection with INT8 quantization and random rotation
 * preprocessing (FHT-based Kac rotation).
 *
 * Key configuration:
 *   quantize_type = ZVEC_QUANTIZE_TYPE_INT8
 *   enable_rotate = true   (NEW: zvec_index_params_set_quantizer_enable_rotate)
 *
 * This triggers the C++ pipeline to:
 *   1. Create a RotationPreprocessor (random orthogonal rotation matrix)
 *   2. Rotate all data vectors before INT8 quantization
 *   3. Store the rotation matrix in the index storage for search-side rotation
 *
 * Input : /root/data/cohere/1m/cohere_train_vector_1m.norm.zvec.vecs
 * Output: /root/data/cohere/1m/db/cohere_cosine_int8_rotate_c
 *
 * Build:
 *   cd examples/c_api && mkdir build && cd build
 *   cmake .. -DHOST_BUILD_DIR=../../../build
 *   make c_api_hnsw_int8_rotate_build
 *
 * Usage:
 *   ./c_api_hnsw_int8_rotate_build
 */
...
#define COLLECTION_PATH "/root/data/cohere/1m/db/cohere_cosine_int8_rotate_c"
 ...
 *   uint16_t  _pad2
 *   uint32_t  meta_size
 *   uint64_t  _reserved[11]
 *
 * Field offsets (pairs of offset, size):
 *   key:       vals[5], vals[6]
 *   dense:     vals[7], vals[8]
 */
...
static int remove_dir_recursive(const char *path) {
    /* Use system() for simplicity — not for production */
    char cmd[2048];
...
int main(void) {
    printf("============================================================\n");
    printf("  Zvec C API — INT8 + Rotate Build Example\n");
    printf("============================================================\n");
    ...
    printf("  enable_rotate = true\n");
    printf("  metric_type   = COSINE\n");

    /* Index params: HNSW + INT8 + rotation */
    ...
    zvec_collection_schema_t *schema =
        zvec_collection_schema_create("cohere_cosine_int8_rotate_c");
    ...
    /* ---- Step 5: Optimize (build HNSW + INT8 + rotation) ---- */
    printf("\n[Step 5] Optimizing collection (HNSW + INT8 + rotation) ...\n");
    ...
    printf("  Run c_api_hnsw_int8_rotate_query to search.\n");
    printf("============================================================\n");
