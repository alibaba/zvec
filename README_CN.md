...
- **稠密 + 稀疏向量**：支持稠密向量、稀疏向量与多向量查询；索引丰富，提供多种[向量索引类型](https://zvec.org/zh/docs/db/concepts/vector-index/#向量索引类型)，覆盖纯内存到磁盘级。
...
- **[Dart/Flutter](https://pub.dev/packages/zvec)**：面向 Flutter 应用的 FFI 绑定，支持 Android 与 iOS。
