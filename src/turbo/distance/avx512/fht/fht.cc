...
void fht_rotate_avx512(const float * /*in*/, float *out, size_t in_dim,
  ...
  if (trunc_dim == dim) {
    fht_flip_sign_avx512(flip, out, dim);
    fht_inplace_avx512(out, trunc_dim);
    fht_vec_rescale_avx512(out, trunc_dim, fac);

    fht_flip_sign_avx512(flip + flip_offset, out, dim);
    fht_inplace_avx512(out, trunc_dim);
    fht_vec_rescale_avx512(out, trunc_dim, fac);

    fht_flip_sign_avx512(flip + 2 * flip_offset, out, dim);
    fht_inplace_avx512(out, trunc_dim);
    fht_vec_rescale_avx512(out, trunc_dim, fac);

    fht_flip_sign_avx512(flip + 3 * flip_offset, out, dim);
    fht_inplace_avx512(out, trunc_dim);
    fht_vec_rescale_avx512(out, trunc_dim, fac);
    return;
  }
  ...
  fht_inplace_avx512(out, trunc_dim);
  fht_vec_rescale_avx512(out, trunc_dim, fac);
  fht_kacs_walk_avx512(out, dim);

  fht_flip_sign_avx512(flip + flip_offset, out, dim);
  fht_inplace_avx512(trunc_ptr, trunc_dim);
  fht_vec_rescale_avx512(trunc_ptr, trunc_dim, fac);
  fht_kacs_walk_avx512(out, dim);
  ...
  fht_vec_rescale_avx512(out, dim, 0.25f);
}
...
void fht_unrotate_avx512(const float * /*in*/, float *out, size_t in_dim,
  ...
  if (trunc_dim == dim) {
    for (int round = 3; round >= 0; --round) {
      fht_inplace_avx512(out, trunc_dim);
      fht_vec_rescale_avx512(out, trunc_dim, fac);
      fht_flip_sign_avx512(flip + static_cast<size_t>(round) * flip_offset, out,
                           dim);
    }
    return;
  }

  fht_vec_rescale_avx512(out, dim, 4.0f);

  size_t start = dim - trunc_dim;
  float *trunc_ptr = out + start;

  fht_inv_kacs_walk_avx512(out, dim);
  fht_inplace_avx512(trunc_ptr, trunc_dim);
  fht_vec_rescale_avx512(trunc_ptr, trunc_dim, fac);
  fht_flip_sign_avx512(flip + 3 * flip_offset, out, dim);
