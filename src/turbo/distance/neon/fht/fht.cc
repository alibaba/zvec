...
void fht_rotate_neon(const float * /*in*/, float *out, size_t in_dim,
  ...
  if (trunc_dim == dim) {
    fht_flip_sign_neon(flip, out, dim);
    scalar::fht_inplace(out, trunc_dim);
    fht_vec_rescale_neon(out, trunc_dim, fac);

    fht_flip_sign_neon(flip + flip_offset, out, dim);
    scalar::fht_inplace(out, trunc_dim);
    fht_vec_rescale_neon(out, trunc_dim, fac);

    fht_flip_sign_neon(flip + 2 * flip_offset, out, dim);
    scalar::fht_inplace(out, trunc_dim);
    fht_vec_rescale_neon(out, trunc_dim, fac);

    fht_flip_sign_neon(flip + 3 * flip_offset, out, dim);
    scalar::fht_inplace(out, trunc_dim);
    fht_vec_rescale_neon(out, trunc_dim, fac);
    return;
  }
  ...
  scalar::fht_inplace(out, trunc_dim);
  fht_vec_rescale_neon(out, trunc_dim, fac);
  fht_kacs_walk_neon(out, dim);

  fht_flip_sign_neon(flip + flip_offset, out, dim);
  scalar::fht_inplace(trunc_ptr, trunc_dim);
  fht_vec_rescale_neon(trunc_ptr, trunc_dim, fac);
  fht_kacs_walk_neon(out, dim);
  ...
  fht_vec_rescale_neon(out, dim, 0.25f);
}
...
void fht_unrotate_neon(const float * /*in*/, float *out, size_t in_dim,
  ...
  if (trunc_dim == dim) {
    for (int round = 3; round >= 0; --round) {
      scalar::fht_inplace(out, trunc_dim);
      fht_vec_rescale_neon(out, trunc_dim, fac);
      fht_flip_sign_neon(flip + static_cast<size_t>(round) * flip_offset, out,
                         dim);
    }
    return;
  }

  fht_vec_rescale_neon(out, dim, 4.0f);

  size_t start = dim - trunc_dim;
  float *trunc_ptr = out + start;

  fht_inv_kacs_walk_neon(out, dim);
  scalar::fht_inplace(trunc_ptr, trunc_dim);
  fht_vec_rescale_neon(trunc_ptr, trunc_dim, fac);
  fht_flip_sign_neon(flip + 3 * flip_offset, out, dim);
