...
int PqInt8Quantizer::init(const IndexMeta &meta, const ailego::Params &params) {
  ...
  l2_batch_fn_ = get_batch_distance_func(
      MetricType::kSquaredEuclidean, input_data_type_,
      input_data_type_ == DataType::kFp16 ? QuantizeType::kFp16
                                          : QuantizeType::kFp32,
      CpuArchType::kAuto);
  ...
  if (meta_.metric_name() == "Cosine") {
    ...
    batch_fn_ = get_batch_distance_func(mt, input_data_type_,
                                        input_data_type_ == DataType::kFp16
                                            ? QuantizeType::kFp16
                                            : QuantizeType::kFp32,
                                        CpuArchType::kAuto);
...
int PqInt8Quantizer::train(IndexHolder::Pointer holder, int thread_count) {
  ...
  // Distribute sub-quantizers across threads, dispatched by data type.
...
int PqInt8Quantizer::dequantize(const void *in, const IndexQueryMeta &qmeta,
  ...
  for (uint32_t m = 0; m < num_chunk_; ++m) {
    ...
    const uint8_t *centroid =
        centroids_m + static_cast<size_t>(code[m]) * d * elem_sz;
