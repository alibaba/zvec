...
class VamanaContext : public IndexContext {
 public:
  ...
  // Candidate output is scoped to a single runner call. Ordinary searches
  // retain their document output and the existing top-k heap path.
  ...
  bool copy_pool_candidates(const Pool &pool, const Entity &entity) {
    ...
    // Heap sorting does not define a stable order for equal distances. Keep
    // that exact path for ties (including a tied cutoff), without re-searching.
    for (int32_t i = 1; i < static_cast<int32_t>(pool.size()); ++i) {
      if (!(pool.dist(i - 1) < pool.dist(i))) return false;
    }
    const uint32_t count = std::min(topk_, static_cast<uint32_t>(pool.size()));
    candidate_keys_->reserve(count);
    for (uint32_t i = 0; i < count; ++i) {
      if (pool.dist(i) > this->threshold()) break;
      candidate_keys_->push_back(entity.get_key(pool.id(i)));
    }
    pool_candidates_ready_ = true;
    return true;
  }
