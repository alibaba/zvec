...
int IVFSearcher::search_impl(const void *query, const IndexQueryMeta &qmeta,
  ...
  if (count == 1) {
    // Single query: use original query-first path
    ctx->reset_results(1);
  ...
  // Build cluster_id -> list of query indices
  struct QueryRef {
    uint32_t q_idx;
  ...
  for (size_t cid : sorted_cids) {
    ...
    // Single call scans the cluster once for all queries
    uint32_t scan_count = 0;
    if (!filter.is_valid()) {
