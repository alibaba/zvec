 ...
 public:
  ...
  virtual std::string debug_string(void) const override {
    ...
    size_t size = snprintf(
        buf, sizeof(buf),
        "scan_cnt=%zu,pairwise_dist_cnt=%zu,get_vector_cnt=%u,get_neighbors_cnt=%u,dup_node=%u",
        get_scan_num(), get_pairwise_dist_num(),
        stats_get_vector_cnt_, stats_get_neighbors_cnt_,
