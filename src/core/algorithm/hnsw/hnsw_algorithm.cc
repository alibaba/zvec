...
// ============================================================================
// search_neighbors: Execute with the concrete heap selected by the caller.
// TopkHeap uses the dual-heap kernel; BlockHeap/LinearPool use the fast kernel.
// ============================================================================
template <typename EntityType>
  ...
  if constexpr (std::is_same_v<Heap, TopkHeap>) {
    ...
    if (index_filter.is_valid()) {
      ...
      dual_heap_search_neighbors<EntityType, MemBlockType>(
          entity, level, entry_point, dist, heap, ctx, dc, visit, filter);
    } else {
      dual_heap_search_neighbors<EntityType, MemBlockType>(
