...
/*! Converter for Uniform Uint7 Quantization (Global Scale)
 *
...
class UniformUint7StreamingConverter : public IndexConverter {
 public:
  ...
  int init(const IndexMeta &index_meta, const ailego::Params &params) override {
    ...
    *stats_.mutable_transformed_count() = 0;
    ...
    metric_params.set(UNIFORM_UINT7_METRIC_ORIGIN_METRIC_NAME,
                      index_meta.metric_name());
    meta_.set_metric("UniformUint7", 0, metric_params);

    const bool has_scale = params.get(UNIFORM_UINT7_REFORMER_SCALE, &scale_);
    const bool has_bias = params.get(UNIFORM_UINT7_REFORMER_BIAS, &bias_);
  ...
  int train(IndexHolder::Pointer holder) override {
    ...
    *stats_.mutable_trained_count() = 0;
    ...
    conv_params.set(UNIFORM_UINT7_REFORMER_SCALE, scale_);
    conv_params.set(UNIFORM_UINT7_REFORMER_BIAS, bias_);
  ...
  int transform(IndexHolder::Pointer holder) override {
    ...
    *stats_.mutable_transformed_count() += holder->count();
    holder_ = std::make_shared<UniformUint7Holder>(holder, original_dimension_,
                                                   scale_, bias_);
    return 0;
  }
 ...
 private:
  //! IndexHolder that applies uniform uint7 quantization on-the-fly
  class UniformUint7Holder : public IndexHolder {
   public:
    class Iterator : public IndexHolder::Iterator {
     public:
      Iterator(const UniformUint7Holder *owner,
     ...
     private:
      void encode_record(void) {
        ...
        for (size_t i = 0; i < dim; ++i) {
          // Match _mm512_cvtps_epi32 used by the SIMD kernel under the
          // default floating-point rounding mode.
          float v = std::nearbyint(vec[i] * scale + bias);
