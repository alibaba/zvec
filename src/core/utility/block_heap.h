...
struct BlockHeap {
  ...
  // `capacity` is the retained top-k size, `block_size` is an upper bound on
  // the per-call push_block size (used only for capacity hints).
  void reset(int32_t n, int32_t capacity, int32_t block_size);
