...
class ZVEC_AILEGO_API LoggerBroker {
 ...
 private:
  ...
  //! Members (Meyers singleton to avoid multiple __cxa_atexit registrations
  //! when logger.cc is compiled into multiple shared libraries via
  //! --whole-archive; the guard variable is shared across all modules through
  //! .dynsym, ensuring initialization and destruction happen exactly once)
  static int &logger_level_() {
    static int level = Logger::LEVEL_WARN;
    return level;
  }
  static Logger::Pointer &logger_() {
    static Logger::Pointer instance;
    return instance;
  }
};
