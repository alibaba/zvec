...
def run_point(
    ...
    return {
        ...
        "read_mib": (
            done["memory"].get("read_bytes", 0) - first["memory"].get("read_bytes", 0)
        )
        / MIB,
...
def run_points(
    args: argparse.Namespace, points: list[tuple[str, int, str, Path]]
) -> list[dict[str, Any]]:
...
def summarize_rows(
    ...
    modes = [("mmap", args.mmap_memory_mb)]
    modes.extend(("buffer_pool", value) for value in args.buffer_memory_mb)
    for workload in args.workloads:
        for mode, memory_mb in modes:
            group = [
                row
                for row in rows
            ...
            summary: dict[str, Any] = {
                ...
                "runs": len(group),
                "qps_stdev": (
                    statistics.stdev(row["qps"] for row in group)
                    if len(group) > 1
                    else 0.0
                ),
            }
            summary.update(
                {
                    f"median_{field}": statistics.median(row[field] for row in group)
    ...
    mmap_by_workload = {
        row["workload"]: row for row in summary_rows if row["mode"] == "mmap"
    }
    ...
    if args.dry_run:
        for point in points:
            for _ in range(args.runs):
                write_line(shlex.join(command_for(args, *point)))
        return

    args.output_dir.mkdir(parents=True, exist_ok=True)
    rows = run_points(args, points)
    csv_path = args.output_dir / "matrix.csv"
    write_csv(csv_path, rows)
    summary_rows = summarize_rows(args, rows)
