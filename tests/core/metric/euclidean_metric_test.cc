...
void CheckRawFp16RefineBatches(turbo::CpuArchType arch,
  ...
  for (size_t dimension :
    ...
    for (size_t count = 0; count <= kCount; ++count) {
      ...
      if (use_metric && dimension >= 512 && count == 12 && flags.AVX2 &&
          flags.F16C && !flags.AVX512_FP16) {
        // This dispatch must be available directly from the metric, without
        // a Flat entity wrapping its batch function.
        std::array<float, 12> expected_batch;
        auto batch12 = reinterpret_cast<IndexMetric::MatrixBatchDistanceHandle>(
            ailego::BaseDistance<ailego::SquaredEuclideanDistanceMatrix,
                                 ailego::Float16, 12, 2>::ComputeBatch);
        batch12(rows.data(), query.data() + 1, count, dimension,
...
TEST(SquaredEuclideanMetric, RawFp16MetricBatchDispatch) {
  CheckRawFp16RefineBatches(turbo::CpuArchType::kScalar, true);
}
