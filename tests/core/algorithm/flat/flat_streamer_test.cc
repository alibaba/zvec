  ...
  for (const auto &[type, metric_name] : cases) {
    for (uint32_t dimension : {128U, 960U}) {
      ...
      for (bool filtered : {false, true}) {
        ...
        for (uint32_t count : {0,  1,  2,  3,  4,  7,  8,  9,  10, 11, 12, 13,
          ...
          ASSERT_EQ(0, streamer->search_bf_by_p_keys_impl(query.data(), keys,
                                                          qmeta, 1, context));
          const auto legacy_result = context->result();
          ASSERT_EQ(0, streamer->search_bf_by_p_keys_impl(query.data(), keys[0],
                                                          qmeta, context));
          ASSERT_EQ(legacy_result.size(), context->result().size());
          for (size_t i = 0; i < legacy_result.size(); ++i) {
            EXPECT_EQ(legacy_result[i].key(), context->result()[i].key());
            EXPECT_FLOAT_EQ(legacy_result[i].score(),
      ...
      for (const auto &key_set : batch_keys) {
        ASSERT_EQ(0, streamer->search_bf_by_p_keys_impl(query.data(), key_set,
                                                        qmeta, context));
