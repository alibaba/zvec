...
static void expect_simd_near(float actual, float expected,
  ...
  // AVX512-FP16 and the NEON FP16 kernels perform their fused multiply-add
  // accumulation in FP16.  The other implementations widen inputs and
  // accumulate in FP32.
  float relative_tolerance = 1.0e-4f;
  if (arch == turbo::CpuArchType::kAVX512FP16) {
    relative_tolerance = 5.0e-3f;
  } else if (arch == turbo::CpuArchType::kNEON) {
    relative_tolerance = 2.0e-2f;
  }
