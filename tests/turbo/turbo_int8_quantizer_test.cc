...
// Record tail size for int8: 4 floats + 1 int32.
static constexpr size_t kInt8TailSize = 20;
  ...
  for (const auto &metric : metrics) {
    for (size_t dim : dimensions) {
      ...
      const auto scalar = turbo::get_distance_kernels(
          metric.type, turbo::DataType::kInt8, turbo::QuantizeType::kRecord,
          turbo::CpuArchType::kScalar);
      ASSERT_TRUE(scalar.dist);
      ASSERT_TRUE(scalar.batch);
      const auto simd =
          turbo::get_distance_kernels(metric.type, turbo::DataType::kInt8,
      ...
      // Int8 kernels take the full encoded size in bytes.
      const size_t kernel_dim = quantizer->quantized_datapoint_vector_length();
...
TEST(Int8Quantizer, General) {
  ...
  ASSERT_EQ(0, quantizer->init(meta, params));
  EXPECT_EQ(DIMENSION + kInt8TailSize + sizeof(float),
...
TEST(Int8Quantizer, Score) {
  ...
  for (size_t i = 1; i < COUNT; ++i) {
    ...
    EXPECT_NEAR(d, expected, 2e-2) << "i=" << i;
  }
  ...
  // --- calc_distance_dp_query_batch ---
  {
    ...
    for (size_t i = 0; i < dp_list.size(); ++i) {
      ...
      EXPECT_NEAR(results[i], expected, 2e-2) << "i=" << i;
    }
  }

  // --- distance() + DistanceImpl (single + batch fallback) ---
  {
    IndexQueryMeta qmeta;
    qmeta.set_meta(IndexMeta::DataType::DT_INT8, DIMENSION,
                   static_cast<uint32_t>(turbo::QuantizeType::kRecord),
                   kInt8TailSize + sizeof(float));
    ...
    // Batch via DistanceImpl (falls back to the single-pair kernel when the
    // batch kernel requires query preprocessing).
    std::vector<const void *> dp_list(COUNT - 1);
    for (size_t i = 1; i < COUNT; ++i) {
      dp_list[i - 1] = quant_vecs[i].data();
    }
    ...
    for (size_t i = 0; i < dp_list.size(); ++i) {
      ...
      EXPECT_NEAR(batch_results[i], expected, 2e-2) << "i=" << i;
    }
  }
...
TEST(Int8Quantizer, ScoreSquaredEuclidean) {
  std::mt19937 gen(7);
  std::uniform_real_distribution<float> dist(0.0, 1.0);
  ...
  zvec::ailego::Params params;
  ASSERT_EQ(0, quantizer->init(meta, params));
  EXPECT_EQ(DIMENSION + kInt8TailSize,
            quantizer->quantized_datapoint_vector_length());
  ...
  for (size_t i = 1; i < COUNT; ++i) {
    ...
    float d = quantizer->calc_distance_dp_query(quant_vecs[i].data(),
                                                quant_vecs[0].data());
    EXPECT_NEAR(d, expected, 5e-2) << "i=" << i;
    EXPECT_NEAR(results[i - 1], expected, 5e-2) << "i=" << i;

    float du = quantizer->calc_distance_dp_query_unquantized(
        quant_vecs[i].data(), raw_vecs[0].data());
    EXPECT_NEAR(du, expected, 5e-2) << "i=" << i;
  }
}

TEST(Int8Quantizer, Avx2DistanceMatchesScalar) {
  check_simd_distance_matches_scalar(turbo::CpuArchType::kAVX2);
}

TEST(Int8Quantizer, SseDistanceMatchesScalar) {
  check_simd_distance_matches_scalar(turbo::CpuArchType::kSSE);
}

TEST(Int8Quantizer, Avx512DistanceMatchesScalar) {
