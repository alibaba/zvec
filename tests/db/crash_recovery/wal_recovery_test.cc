...
class WalRecoveryDeathTest : public ::testing::Test {
 protected:
  void SetUp() override {
  ...
  const std::string path_{"wal_recovery_regression_db"};
};

TEST_F(WalRecoveryDeathTest, LargeStringAndTrailingDocRecoverFromWal) {
  const std::vector<std::string> ids{"prefix", "large", "suffix"};
  // Exceed the former reader-only limit using an ordinary short ID.
  const std::vector<std::string> values{
      "before", std::string(4 * 1024 * 1024 + 1024, 'v'), "after"};
  ::testing::FLAGS_gtest_death_test_style = "threadsafe";
...
TEST_F(WalRecoveryDeathTest, IncompleteTailAllowsLaterCrashRecovery) {
  ::testing::FLAGS_gtest_death_test_style = "threadsafe";
  ASSERT_EXIT(
...
TEST_F(WalRecoveryDeathTest, CorruptWalFailsOpenWithoutReplacingFiles) {
  ::testing::FLAGS_gtest_death_test_style = "threadsafe";
  ASSERT_EXIT(
...
TEST_F(WalRecoveryDeathTest, CorruptTailDoesNotApplyUpsertPrefix) {
  ::testing::FLAGS_gtest_death_test_style = "threadsafe";
...
TEST_F(WalRecoveryDeathTest, UpsertReplayIgnoresSameAndLaterReplayIds) {
  ::testing::FLAGS_gtest_death_test_style = "threadsafe";
...
TEST_F(WalRecoveryDeathTest, UpsertWalRecordsInsertOrUpdatePredecessor) {
  ASSERT_EXIT(WriteUpsertsAndExit(path_, false, false),
              ::testing::ExitedWithCode(0), "");
  std::vector<Doc::Ptr> docs;
  ASSERT_NO_FATAL_FAILURE(ReadWalDocuments(path_, &docs));
