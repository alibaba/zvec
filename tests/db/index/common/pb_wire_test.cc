...
TEST(PbWireWriterTest, VarintRoundTrip) {
  ...
  for (const auto &test_case : cases) {
    std::string buf;
    Writer writer(&buf);
    writer.put_varint(1, test_case.value);
...
TEST(PbWireWriterTest, DefaultValuesAreSkipped) {
  std::string buf;
  Writer writer(&buf);
  writer.put_varint(1, 0);
  writer.put_bool(2, false);
  writer.put_float(3, 0.0f);
  writer.put_string(4, "");
...
TEST(PbWireWriterTest, FloatRoundTrip) {
  std::string buf;
  Writer writer(&buf);
  writer.put_float(5, 3.5f);
  writer.put_float(6, -1.25f);
...
TEST(PbWireWriterTest, MultipleFieldsInOrder) {
  std::string buf;
  Writer writer(&buf);
  writer.put_varint(1, 42);
  writer.put_string(2, "zvec");
  writer.put_bool(3, true);
...
TEST(PbWireWriterTest, Int32AndUint32ValueInterpretation) {
  ...
  writer.put_varint(1, 0xFFFFFFFFULL);
  // Larger than 32 bits: uint32_value keeps the low 32 bits.
  writer.put_varint(2, 0x1FFFFFFFFULL);
