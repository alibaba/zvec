...
TEST_F(CollectionTest, Feature_QueryWithSchema_ConsistentWithQuery) {
  FileHelper::RemoveDirectory(col_path);
  ...
  auto snapshot = collection->QueryWithSchema(query);
...
TEST_F(CollectionTest, Feature_QueryWithSchema_AtomicUnderDropColumn) {
  FileHelper::RemoveDirectory(col_path);
  ...
  auto check_snapshot = [&](const Result<QuerySnapshot> &result) {
  ...
  std::thread drop_thread([&]() {
    ...
    auto s = collection->DropColumn(dropped_field);
    ASSERT_TRUE(s.ok());
    dropped = true;
  });

  // Hammer QueryWithSchema from two threads while the DDL runs. A small
  // pause between queries keeps the shared lock from starving the DDL.
  std::thread reader([&]() {
    while (!stop) {
      check_snapshot(collection->QueryWithSchema(query));
  ...
  auto final_result = collection->QueryWithSchema(query);
