...
function(_setup_armv8_march)
  ...
  # GCC only accepts "armv8-a"; Clang accepts both "armv8-a" and "armv8".
  set(_armv8_flags "armv8-a" "armv8")
  foreach(_arch IN LISTS _armv8_flags)
    string(MAKE_C_IDENTIFIER "_COMP_SUPP_${_arch}" _arch_var)
    check_c_compiler_flag("-march=${_arch}" ${_arch_var})
    if(${_arch_var})
...
function(setup_compiler_march_for_arm VAR_NAME_NEON VAR_NAME_NEON_FP16)
  if(MSVC)
    # MSVC ARM64: NEON is the ARMv8 baseline; no GCC-style -march accepted.
    set(${VAR_NAME_NEON} "" PARENT_SCOPE)
    set(${VAR_NAME_NEON_FP16} "" PARENT_SCOPE)
    return()
  endif()

  #neon (ARMv8 baseline)
  set(${VAR_NAME_NEON} "-march=armv8-a" PARENT_SCOPE)

  #neon fp16 (FEAT_FP16: native half-precision vector arithmetic,
  #__ARM_FEATURE_FP16_VECTOR_ARITHMETIC). Kernels compiled with this flag
  #must be runtime-gated on CpuFeatures FP16 by their dispatchers.
  set(_arm_flags_neon_fp16 "armv8.2-a+fp16" "armv8-a")
  foreach(_arch_neon_fp16 IN LISTS _arm_flags_neon_fp16)
    string(MAKE_C_IDENTIFIER "_COMP_SUPP_${_arch_neon_fp16}" _arch_neon_fp16_var)
    check_c_compiler_flag("-march=${_arch_neon_fp16}" ${_arch_neon_fp16_var})
    if(${_arch_neon_fp16_var})
      set(${VAR_NAME_NEON_FP16} "-march=${_arch_neon_fp16}" PARENT_SCOPE)
      break()
    endif()
  endforeach()
endfunction()
